...
func TestExecuteCreate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			repoBranchName:   "main",
			prCreate:         "pull request created",
			expectedErr:      errors.New("No changes found, aborting PR creation"),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			linterArgs = append(linterArgs, ghutil.ConvertTerminalOutputIntoList(tt.modifiedFiles)...)
			...
			mockExe.On("Command", "git", []string{"log", "--oneline", "origin/main.."}).Return("", nil)
